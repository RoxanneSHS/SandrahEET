...
CHAIN
IF~ GlobalGT("SanSuBH","GLOBAL",18) Global("GetSwill","GLOBAL",0) ~ THEN Capholg Swill
~ So what, wench, got my refill?~
DO~ SetGlobal("GetSwill","GLOBAL",1) ~
==BSANDR ~ Ay, Captain, on its way. Say, do me a favour and take a look at mine map meantime.~
== Capholg ~ Map, hey, for the finest cap'n' off sea, no problem no, ‰‰h...~
==BSANDR ~ Any idea what it is, captain?~
== Capholg ~ Any idea where me drinkin' is, gal?~
==BSANDR ~ Sure, on its way.~
== Capholg ~ You said likewise earlier, but I not see it. Kant decipher map with tank all empty, gotme, wench?~
==BSANDR ~ Helps not, <CHARNAME>, we got to find the captain something to drink.~
END
...
DO~ SetGlobal("GetSwill","GLOBAL",2) ~
==BSANDR ~ Ay, Captain, any ideas?~
...
==BSANDR ~ Ay, Captain, so you know what we know.~
== Capholg ~ And I know a little bit more. I know where it is - the island. The deadly trap.~
==BSANDR ~ You do not look to me like you were one of them, with all respect, sir.~
...
==BSANDR ~ As you said, we are all in the same boat somehow, we all know about it now. So you might as well tell us the rest.~
...
==BSANDR ~ And they are still all out there, zombies by now?~
...
==BSANDR ~ We have seen a bit of the power of Monteelah ourselves.~
...
==BSANDR ~ You have a ship, captain?~
...
==BSANDR ~ We can surely be your deckhands on that, if you advise us right. We are experienced adventurers even if we are no sailors. But you are acting in your own interest, so why the cost?~
== Capholg ~Lady! I might have been a bit drunk when you found me - but now I am sober. I am a captain of honor and no pirate...~
==BSANDR ~ I am sorry, captain, I was not trying to insult you.~
...
==BSANDR ~ Understood. So, how much are we talking about here?~
== Capholg ~ 20000 Gold.~
END
IF ~ PartyGoldLT(20000)~ THEN REPLY ~ We do not have the money at the moment. We will come back when we are ready to do the journey~ DO~ SetGlobal("GetSwill","GLOBAL",3) ~ EXIT
IF ~ PartyGoldGT(20000)~ THEN REPLY ~ Fine, the money is no problem. But we need some preparations on our own. We will come back when we are ready to do the journey.~ DO~ SetGlobal("GetSwill","GLOBAL",3) ~ EXIT
...
CHAIN
IF ~ Global("GetSwill","GLOBAL",3) ~ THEN Capholg Leave
~ I see you are back. Everything is ready, my friends.~
END
++ ~ The sooner we get this whole thing done, the better. We are ready to go right away.~ DO ~ SetGlobal("GetSwill","GLOBAL",4) SetGlobal("SanSuBH","GLOBAL",25) TakePartyGold(20000)
...
CHAIN
IF ~ Global("GetSwill","GLOBAL",5) AreaCheck("CVPIR1") ~ THEN Capholg waitboat
~ Everything is ready, just come on board.~
DO~ SetGlobal("GetSwill","GLOBAL",6)~
END
...
CHAIN
IF ~ Global("GetSwill","GLOBAL",6) AreaCheck("CVPIR2") ~ THEN Capholg waitisl
