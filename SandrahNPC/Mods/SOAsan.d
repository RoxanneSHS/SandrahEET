...
SAY~Some items could tell long stories if they had a mouth - like for example this chainmail.~
IF~~THEN REPLY~ You probably could speak for it, my learned counselor.~ DO~ IncrementGlobal("Sanpoints","GLOBAL",1) SetGlobal("Mecorima","ars002",3)~GOTO SanMecorima2
...
=~It is reported that agents of the stormlord Talos assasinated Mecorima as she disturbed some of their sinister plots. They took the chain mail from her corpse. Its destiny from there was unknown - until today. ~
...
== ARANOR IF ~ InParty("CVSandr")~ THEN~ You know much, respectable young lady. So I assume you are as well aware that the Harpers acted not for their own interests but for the wellfare of the Realms at large. Anyway I will listen to you.~
== BSANDR IF ~ InParty("CVSandr")~ THEN~ We need a bit of lore about a deceased member of the Harpers, Gorion of Candlekeep.~
== ARANOR IF ~ InParty("CVSandr")~ THEN~ Hmm, hmm. Your father, Sandrah of Waterdeep, must have his reasons to let you and your companion, Gorion's Ward <CHARNAME>, come to your own conclusions. I will not interfer with Elminster, even if I had the knowledge you are seeking, which is not the case.~
